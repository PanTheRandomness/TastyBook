...
describe('Image funtionality tests:', () => {
    test('displays error message when wrong file type is selected', async () => {
        const { getByTestId, queryByTestId } = render(<AddRecipe />);

        const fileInput = getByTestId('recipeImageInput');
        ...
        await waitFor(() => {
            ...
            expect(queryByTestId('currentimage')).toBeInTheDocument();
            expect(queryByTestId('wrongimageerror')).toBeNull();
        });
        ...
        await waitFor(() => {
            expect(queryByTestId('wrongimageerror')).toBeInTheDocument();
            expect(queryByTestId('currentimage')).toBeNull();
        });
    });

    test('displays selected image as current image', async () => {
        ...
        await waitFor(() => {
            expect(getByTestId('currentimage')).toBeInTheDocument();
        });
    });

    test('removes current image when remove image button is clicked', async () => {
        const { getByTestId, queryByTestId } = render(<AddRecipe />);
        ...
        const removeImageButton = getByTestId('removeimagebutton');

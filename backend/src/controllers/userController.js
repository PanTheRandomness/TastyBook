...
const signup = async (req, res) => {
    try {
        ...
        const duplicateUser = await sql.findUserByUsername(username);
        ...
        if (notEmpty[0]["EXISTS (SELECT 1 FROM user)"]) result = result = await sql.addUser(username, name, email, passwordHash, "waiting");
        else result = await sql.addUser(username, name, email, passwordHash, "registered", 1);
...
// Signs jwt with user's id, username, and current time
